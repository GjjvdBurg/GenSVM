function test_get_zaz_zb()
  ...
  Q = calculate_errors(ZV, U, y, n, K);
  H = calculate_huber(Q, n, K, kappa);

  LZ = zeros(n, m+1);
  ZB = zeros(m+1, K-1);

  for i=1:n
    [thealpha, thebeta] = calculate_alpha_beta(Q, H, U, y, rho, K, ...
      kappa, p, i);
    fprintf('i = %i, alpha = %.16f, beta = [', i, thealpha);
    for gjj=1:K-1
      fprintf(' %.16f', thebeta(gjj));
    end
    fprintf(' ]\n');

    sqalpha = sqrt(thealpha);

    LZ(i, 1) = sqalpha;
    LZ(i, 2:end) = sqalpha * Z(i, 2:end) + LZ(i, 2:end);
  ...
  for i=1:size(ZAZ, 1)
    for j=i:size(ZAZ, 2)
      fprintf(['mu_assert(fabs(matrix_get(work->ZAZ, m+1, m+1, %i, %i) -\n', ...
        '%.16f) < 1e-14,\n"Incorrect value for ZAZ at %i, %i");\n'], ...
        i-1, j-1, ZAZ(i, j), i-1, j-1);
    end
  end
  fprintf('\n');
  ...
  for i=1:size(ZB, 1)
    for j=1:size(ZB, 2)
      fprintf(['mu_assert(fabs(matrix_get(work->ZB, m+1, K-1, %i, %i) -\n', ...
        '%.16f) < 1e-14,\n"Incorrect value for ZB at %i, %i");\n'], ...
        i-1, j-1, ZB(i, j), i-1, j-1);
    end
  end
...
function [thealpha, thebeta] = calculate_alpha_beta(Q, H, U, y, rho, K, ...
    kappa, p, i)
  thebeta = zeros(1, K-1);
  thealpha = 0.0;
  n = size(Q, 1);

  is_simple = majorize_is_simple(H, y, K, i);
  if (is_simple)
    omega = 1.0;
  else
    omega = calculate_omega(H, y, K, p, i);
  end
...
function omega = calculate_omega(H, y, K, p, i)
  omega = 0.0;
  for j=1:K
    if (j == y(i))
      continue
...
function is_simple = majorize_is_simple(H, y, K, i)
  is_simple = true;
  value = 0;
  for j=1:K
    if (j == y(i))
...
function Q = calculate_errors(ZV, U, y, n, K)
  % Calculate errors
  Q = zeros(n, K);
  for i=1:n
    for j=1:K
...
function H = calculate_huber(Q, n, K, kappa)
  % Calculate huber
  H = zeros(n, K);
  for i=1:n
    for j=1:K
